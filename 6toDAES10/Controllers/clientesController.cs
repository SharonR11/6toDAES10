...
using _6toDAES10.Models;

namespace _6toDAES10.Controllers
{
    public class clientesController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var ClientesActivos = db.clientes.Where(
                c => c.Estado == true
                ).ToList();
        ...
        public ActionResult Edit([Bind(Include = "idCliente,NombreCompañia,NombreContacto,CargoContacto,Direccion,Ciudad,Region,CodPostal,Pais,Telefono,Fax")] clientes clientes)
        {
            if (ModelState.IsValid)
            {
                var originalClient = db.clientes.AsNoTracking().FirstOrDefault(
                    c => c.idCliente == clientes.idCliente);
                if (originalClient != null)
                {
                    clientes.Estado = originalClient.Estado; 
                }
